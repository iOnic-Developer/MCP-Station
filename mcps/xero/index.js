...
// Xero split its scopes: apps created before 2 Mar 2026 carry BROAD scopes (until Sep 2027);
// apps created after are GRANULAR-only. We can't know which an org's Custom Connection has, so
// (like Xero's own MCP server) we try broad first and fall back to granular on invalid_scope.
const SCOPES_BROAD = 'accounting.transactions accounting.contacts accounting.settings accounting.reports.read payroll.settings payroll.employees payroll.timesheets';
const SCOPES_GRANULAR = 'accounting.invoices accounting.payments accounting.banktransactions accounting.manualjournals accounting.reports.aged.read accounting.reports.balancesheet.read accounting.reports.profitandloss.read accounting.reports.trialbalance.read accounting.reports.banksummary.read accounting.reports.executivesummary.read accounting.contacts accounting.settings payroll.settings payroll.employees payroll.timesheets payroll.payruns';

// Get a client_credentials token, auto-negotiating the scope vintage.
// Order: user override (if set) → broad → granular. Only advances on invalid_scope; any other
// failure (invalid_client, network) throws immediately. Returns the raw token response.
async function requestXeroToken(fetchJson, clientId, clientSecret, userScopes) {
  const basic = btoa(`${clientId}:${clientSecret}`);
  const attempts = [];
  if (userScopes && userScopes.trim()) attempts.push(userScopes.trim());
  attempts.push(SCOPES_BROAD, SCOPES_GRANULAR);
  const tried = new Set();
  let lastErr, allScopeErrors = true;
  for (const scope of attempts) {
    if (tried.has(scope)) continue;
    tried.add(scope);
    try {
      return await fetchJson(`${IDENTITY_BASE}/connect/token`, {
        method: 'POST',
      ...
      lastErr = e;
      if (!/invalid_scope|No valid scopes/i.test(e.message || '')) { allScopeErrors = false; throw e; }
    }
  }
  if (allScopeErrors) {
    throw new Error(`Xero rejected every scope set (invalid_scope) — the Custom Connection has none of the requested scopes enabled. On developer.xero.com open the Custom Connection, TICK the scopes you want and Save, then retry. For full module capability tick: ${SCOPES_GRANULAR}`);
  }
  throw lastErr;
}
...
export function register({ server, z, getSettings, log, fetchJson }) {
  async function token() {
    ...
    let tok;
    try {
      tok = await requestXeroToken(fetchJson, client_id, client_secret, scopes);
    } catch (e) {
      log(`xero: token request failed — ${e.message}`);
      if (/invalid_scope|No valid scopes/i.test(e.message)) throw e; // already actionable
      throw new Error(`Xero token request failed: ${e.message}. Check client_id/client_secret and that the app is a Custom Connection (invalid_client = wrong app type or bad secret; unauthorized_client = not authorised for client_credentials).`);
    }
...
Args:
  ...
  log('xero module registered (31 tools, scope auto-negotiate broad→granular)');
}
...
export async function test(settings, { fetchJson }) {
  ...
  try {
    const tok = await requestXeroToken(fetchJson, settings.client_id, settings.client_secret, settings.scopes);
