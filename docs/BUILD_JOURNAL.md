...
| 13 | n8n joins as a bundled default module 🔀 (n8n v1.0.0, slug `n8n_mcp`, station v1.6.0): **66 tools covering all 103 endpoints** of the n8n Public API v1.1.1, mapped from the live instance's own OpenAPI spec (`/api/v1/docs`), not the (thinner) public docs. Optional Cloudflare Access service-token settings (`CF-Access-Client-Id`/`-Secret` headers). Notables: `PUT /workflows/{id}` demands the full body → update tool does fetch-merge-PUT; MCP `structuredContent` must be an object → arrays from n8n get wrapped `{result}`; tag names >24 chars surface n8n's misleading `409 Tag already exists` → schema caps at 24; package export/import are binary (gzip/multipart) → Node global `fetch`+`FormData`, base64 through chat. Verified live against n8n.dbzocchi.app: 33/33 harness checks (read surface + self-cleaning write cycle: workflow/tag/data-table CRUD, archive/unarchive, history, package export), license-gated areas (variables/projects/policy) error readably; station boot 9/9 modules, 66 tools over HTTP | — |
| 14 | **✦ assistant rebuilt around streaming + in-place edits (station v1.8.0)** after David reported the module chat going quiet and answering with snippets/whole files at random. Root causes: non-streamed upstream hops (minutes of silence → Cloudflare's 100 s read timeout closed the response, the UI saw a stream that just ended), `max_tokens: 8192` with a brief that demanded complete files (n8n's index.js alone is ~18k tokens — cut off silently, or a tool call truncated to nothing), and a 60k-char source budget that hid the end of big files. Now: SSE streaming on both providers + keep-alive comments, per-model output cap from the Models API with 400/429 self-heal, a `notice` event for cut-offs, `read/edit/write_module_file` tools (exact find/replace, hot-reload, load status back to the model), editor tab auto-refresh on `file_changed`, ⤵ Insert → ⤵ Replace file with a snippet guard, `fetch_url` + an endpoint-inventory workflow so modules built from docs cover every endpoint. Dashboard redesigned as two columns of one-line rows with icon-only grouped buttons, status-coloured ⚙/▶, client count + token dot on 🔑, delete moved into Settings. Verified: `scripts/smoke-assistant.sh` 57/57 against `scripts/mock-llm.mjs`, `smoke.sh` 36/36, oauth 34/34, scoping 15/15, self-contained 10/10, plus Playwright runs of the editor chat (13/13) and the list (22/22) in headless Chromium | — |
