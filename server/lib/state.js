...
export function gc() {
  ...
  for (const [k, v] of Object.entries(st.oauth.codes)) if (v.expiresAt < now) { delete st.oauth.codes[k]; dirty = true; }
  for (const [k, v] of Object.entries(st.oauth.tokens)) if (v.expiresAt < now) { delete st.oauth.tokens[k]; dirty = true; }
  for (const [k, v] of Object.entries(st.oauth.refresh)) if (v.expiresAt < now) { delete st.oauth.refresh[k]; dirty = true; }
  if (dirty) save();
}
